...
class audio_sample: 
    ...
    def gen_mel_bank(cls, n_filterbanks=26, n_fft=1024, low_freq=200, high_freq=16000):
        ...
        cls.mel_bank = np.zeros([int((n_fft/2)), n_filterbanks]) # set up mel bank matrix (zeros)
        ...
        for m in range(n_filterbanks):
            for k in range(int(filter_bins[m]), int(filter_bins[m+1])): # between previous and present filter centre
                cls.mel_bank[k,m] = (k-filter_bins[m])/(filter_bins[m+1] - filter_bins[m])

            for k in range(int(filter_bins[m+1]), int(filter_bins[m+2])): # between present and next filter centre
                cls.mel_bank[k,m] = (filter_bins[m+2]-k)/(filter_bins[m+2] - filter_bins[m+1])
    ...
    def gen_mfccs(self, n_mfccs=11): 
        ...
        self.mfccs = np.zeros([np.size(audio_sample.mel_bank,1), n_frames])
        ...
        for n in range(n_frames):
            ...
            for m in range(np.size(audio_sample.mel_bank,1)): # MFCC calculation loop
